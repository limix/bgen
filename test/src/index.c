...
void create_metafile(char const* bgen_file_filepath, char const* metafile_filepath)
{
    ...
    struct bgen_metafile_04* metafile = bgen_metafile_create_04(bgen_file, metafile_filepath, 2, 0);
    cass_cond(metafile != NULL);
    cass_equal_int(bgen_metafile_close_04(metafile), 0);
    bgen_file_close(bgen_file);
}

void use_metafile_properly(char const* metafile_filepath)
{
    struct bgen_metafile_04* metafile = bgen_metafile_open_04(metafile_filepath);
    cass_cond(metafile != NULL);

    cass_equal_int(bgen_metafile_npartitions_04(metafile), 2);

    cass_equal_int(bgen_metafile_nvariants_04(metafile), 10);
...
void use_metafile_wrongly(char const* metafile_filepath)
{
    struct bgen_metafile_04* metafile = bgen_metafile_open_04(metafile_filepath);
    cass_cond(metafile != NULL);
    cass_cond(bgen_metafile_read_partition_04(metafile, 3) == NULL);
    cass_equal_int(bgen_metafile_close_04(metafile), 0);
}
