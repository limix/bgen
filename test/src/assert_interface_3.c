...
void test_open_wrong_bgen_filepath(void)
{
    struct bgen_file* bgen = bgen_file_open("data/wrong.metafile");
    cass_cond(bgen == NULL);
}
...
void test_create_meteadata_haplotypes(void)
{
    ...
    struct bgen_metafile_04* mf = bgen_metafile_create_04(bgen, "assert_interface_3.tmp/haplotypes.bgen.metafile.1", 4, 0);
    cass_cond(mf != NULL);

    cass_equal_int(bgen_metafile_npartitions_04(mf), 4);
    cass_equal_int(bgen_metafile_nvariants_04(mf), 4);
    ...
    cass_equal_int(bgen_metafile_close_04(mf), 0);
    bgen_file_close(bgen);
}
...
void test_genotype_haplotypes_by_creating_metadata(void)
{
    ...
    struct bgen_metafile_04* mf = bgen_metafile_create_04(bgen, "assert_interface_3.tmp/haplotypes.bgen.metafile.2", 4, 0);
    cass_cond(mf != NULL);
    ...
    for (uint32_t i = 0; i < 4; ++i) {
        struct bgen_partition const* partition = bgen_metafile_read_partition_04(mf, i);
...
void test_genotype_haplotypes_by_loading_metadata(void)
{
    ...
    struct bgen_metafile_04* mf = bgen_metafile_open_04("assert_interface_3.tmp/haplotypes.bgen.metafile.2");
    cass_cond(mf != NULL);
    ...
    cass_equal_int(bgen_metafile_nvariants_04(mf), 4);
    double* probs_ptr = &probs[0];
    for (uint32_t i = 0; i < bgen_metafile_npartitions_04(mf); ++i) {
        struct bgen_partition const* partition = bgen_metafile_read_partition_04(mf, i);
