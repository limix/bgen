...
int main() {
    ...
    if (test_filepath((byte *)"data/example.1bits.bgen", &index) == FAIL)
        return FAIL;

    bgen_free_indexing(index);

    if (test_filepath((byte *)"data/example.32bits.bgen", &index) == FAIL)
        return FAIL;
    ...
    if (test_filepath((byte *)"data/example.v11.bgen", &index) == FAIL)
        return FAIL;
